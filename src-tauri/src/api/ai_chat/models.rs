...
impl ModelProvider for OpenAiProvider {
    ...
    async fn chat_stream(
        ...
        // 将字节流转换为事件流
        let event_stream = byte_stream.map(|chunk_result| -> Vec<Result<StreamEvent, String>> {
                ...
                Ok(chunk) => {
                    ...
                    while let Some(line) = lines.next() {
                        ...
                        if let Some(data) = line.strip_prefix("data: ") {
                            ...
                            } else if let Ok(stream_response) = serde_json::from_str::<OpenAiStreamResponse>(data) {
                                if let Some(choice) = stream_response.choices.first() {
                                    if let Some(content) = &choice.delta.content {
                                        events.push(Ok(StreamEvent::Content { text: content.clone() }));
                                    }
                    ...
                    for line in chunk_str.split('\n') {
                        ...
                        if let Ok(stream_response) = serde_json::from_str::<OllamaStreamResponse>(line) {
                            if let Some(message) = stream_response.message {
                                events.push(Ok(StreamEvent::Content { text: message.content }));
                            }
...
pub async fn send_chat_message(
    ...
    let provider_instance: Box<dyn ModelProvider> = match provider.to_lowercase().as_str() {
        "openai" => {
            ...
            let api_key = keyring.get_api_key(KeyringModelProvider::OpenAi).map_err(|e| e.to_string())?;
            drop(keyring);
        ...
        "anthropic" => {
            let keyring = state.keyring_manager.lock().map_err(|e| e.to_string())?;
            let api_key = keyring.get_api_key(KeyringModelProvider::Anthropic).map_err(|e| e.to_string())?;
            drop(keyring);
    ...
    tokio::spawn(async move {
        while let Some(event_result) = stream.next().await {
            match event_result {
                ...
                Err(e) => {
                    let _ = app.emit(&format!("chat-stream-{}", request_id), &StreamEvent::Error { message: e });
                    break;
                }
            }
        }
