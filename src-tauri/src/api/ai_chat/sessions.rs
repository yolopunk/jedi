...
use crate::utils::security::{AuditLogger, SecurityEvent, SecurityEventType, OperationResult};
...
impl ChatSessionManager {
    ...
    /// 创建会话
    pub fn create_session(&mut self, title: String, provider: String, model: String) -> Result<ChatSession, String> {
    ...
    /// 追加消息到会话
    pub fn append_message(&mut self, session_id: &str, role: String, content: String) -> Result<ChatMessage, String> {
...
pub async fn create_session(
    ...
    let mut manager = state.manager.lock().map_err(|e| format!("Lock error: {}", e))?;
...
mod tests {
    ...
    fn test_session_manager() {
        ...
        // 追加消息
        let message = manager.append_message(
            &session.id,
            "user".to_string(),
            "Hello".to_string(),
        ).unwrap();
