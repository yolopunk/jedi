...
fn parse_subscription_from_channel(channel: &Channel, rss_url: &str) -> PodcastSubscription {
    ...
    let (owner_name, owner_email) = itunes
        ...
        .map(|o| (o.name.clone().unwrap_or_default(), o.email.clone().unwrap_or_default()))
        .unwrap_or_default();
...
fn parse_episodes_from_channel(channel: &Channel) -> Vec<PodcastEpisode> {
    let channel_image = channel.itunes_ext.as_ref()
        .and_then(|ext| ext.image.clone())
        .or_else(|| channel.image.as_ref().map(|i| i.url.clone()));

    channel.items.iter().map(|item| {
        // Audio URL
        let audio_url = item.enclosure.as_ref().map(|e| e.url.clone()).unwrap_or_default();

        // GUID
        let guid = item.guid.as_ref().map(|g| g.value.clone()).unwrap_or_default();

        // Image: Item iTunes > Item Media > Channel Image
        let image_url = item.itunes_ext.as_ref()
        ...
        let show_notes = item.content.clone()
...
#[tauri::command]
pub async fn refresh_subscription(app: AppHandle, rss_url: String) -> Result<PodcastSubscription, String> {
    // 1. Fetch latest info
...
#[tauri::command]
pub async fn save_subscription(app: AppHandle, sub: PodcastSubscription) -> Result<Vec<PodcastSubscription>, String> {
...
#[tauri::command]
pub async fn remove_subscription(app: AppHandle, rss_url: String) -> Result<Vec<PodcastSubscription>, String> {
    let mut subs = get_subscriptions(app.clone()).await?;
...
#[tauri::command]
pub async fn import_opml(app: AppHandle, opml_content: String) -> Result<Vec<PodcastSubscription>, String> {
...
async fn resolve_xiaoyuzhou_rss_url(url: &str) -> Result<String, String> {
    ...
    let html = res.text().await.map_err(|e| format!("Read failed: {}", e))?;
