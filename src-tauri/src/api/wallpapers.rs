...
impl WallpaperManager {
    ...
    pub async fn process_item(&self, name: String, url: String) -> Result<WallpaperItem, String> {
        ...
        let front_matter: FrontMatter = result.data.ok_or_else(|| format!("No frontmatter found for {}", slug))
            .and_then(|data: Pod| data.deserialize::<FrontMatter>().map_err(|e| format!("Failed to parse frontmatter: {}", e)))?;

        let title = front_matter.title.ok_or_else(|| format!("Missing title for {}", slug))?;
        let image = front_matter.image.ok_or_else(|| format!("Missing image for {}", slug))?;
        
        let category = front_matter.categories
            .as_ref()
            .and_then(|c| c.first())
            .cloned()
...
pub async fn sync_wallpapers<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    ...
    let items_to_process: Vec<_> = contents.into_iter()
    ...
    while let Some(result) = stream.next().await {
        if let Some((name, sha, item)) = result {
            cache.insert(name, CacheEntry { sha, item: item.clone() });
...
pub async fn set_desktop_wallpaper<R: Runtime>(
    ...
    let response = manager.client.get(&url).send().await.map_err(|e| e.to_string())?;
