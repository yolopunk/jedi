...
impl HostsMcpServer {
    ...
    pub fn list_tools(&self) -> Result<Vec<Tool>, McpError> {
        ...
        let tools = vec![
            ...
            Tool {
                ...
                description: Some(
                    "读取系统 Hosts 文件内容，返回所有分组和条目".to_string(),
                ),
    ...
    fn handle_hosts_read(&self) -> Result<CallToolResult, McpError> {
        let groups = read_system_hosts().map_err(|e| {
            McpError::Protocol(format!("Failed to read hosts: {}", e))
        })?;

        let result = serde_json::to_string_pretty(&groups).map_err(|e| {
            McpError::Serialization(format!("Failed to serialize hosts: {}", e))
        })?;
    ...
    fn handle_hosts_list(
        ...
        let result = serde_json::to_string_pretty(&filtered_groups).map_err(|e| {
            McpError::Serialization(format!("Failed to serialize hosts: {}", e))
        })?;
    ...
    fn handle_hosts_add(
        ...
        // 读取当前 hosts
        let mut groups = read_system_hosts().map_err(|e| {
            McpError::Protocol(format!("Failed to read hosts: {}", e))
        })?;
    ...
    fn handle_hosts_write(
        ...
        let groups: Vec<GroupHosts> = serde_json::from_value(groups_value.clone()).map_err(|e| {
            McpError::InvalidParams(format!("Invalid groups format: {}", e))
        })?;
    ...
    fn write_hosts(&self, groups: Vec<GroupHosts>) -> Result<(), McpError> {
        ...
        std::fs::write(HOSTS_PATH, new_content).map_err(|e| {
            McpError::Io(format!("Failed to write hosts file: {}", e))
        })?;
