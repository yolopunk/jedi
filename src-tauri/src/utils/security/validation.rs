...
/// 验证 API Key
pub fn validate_api_key(key: &str, provider: Option<&str>, options: Option<&ValidationOptions>) -> ApiKeyValidation {
    let default_opts = ValidationOptions::default();
    let opts = options.unwrap_or(&default_opts);
    let mut warnings = Vec::new();
...
pub fn validate_chat_message(content: &str, is_user: bool) -> ChatMessageValidation {
    ...
    ChatMessageValidation {
        valid: warnings.is_empty()
            || (warnings.len() == 1 && metadata.char_count > 50000),
...
fn is_localhost(host: &str) -> bool {
    matches!(host.to_lowercase().as_str(), "localhost" | "127.0.0.1" | "::1" | "0.0.0.0")
}
...
pub fn sanitize_html(html: &str) -> String {
    ...
    // 移除危险属性
    let dangerous_attrs = [
        "onload", "onerror", "onclick", "onmouseover", "onfocus", "onblur", "onsubmit",
