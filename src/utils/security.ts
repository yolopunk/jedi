...
// 危险的 HTML 标签和属性
const DANGEROUS_TAGS = ['script', 'iframe', 'form', 'input', 'button', 'textarea', 'select', 'option'];
const DANGEROUS_ATTRS = ['onload', 'onerror', 'onclick', 'onmouseover', 'onfocus', 'onblur', 'onsubmit'];
...
export function validateEndpoint(endpoint: string): ValidationResult {
  ...
  try {
    ...
    const isLocal = localPatterns.some(pattern => hostname === pattern || hostname.endsWith('.' + pattern));
 ...
 * @returns 验证结果
 */
export function validateChatMessage(content: string, isUser: boolean = true): ChatMessageValidation {
  const warnings: string[] = [];
  ...
  // 分析内容元数据
  const words = sanitizedContent.trim().split(/\s+/).filter(w => w.length > 0);
  const urlRegex = /https?:\/\/[^\s]+/gi;
